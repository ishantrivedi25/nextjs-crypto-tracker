...
import styles from "./list.module.css"; // Import styles correctly
import { convertNumber } from "@/lib/convertNumber";
import { saveItemToWatchlist } from "@/lib/saveItemToWatchlist";
import { removeItemFromWatchlist } from "@/lib/removeItemFromWatchlist";
      ...
      >
        ...
        <Tooltip title="Coin Info" placement="bottom-start">
          <td className={styles.tdInfo}>
            <div className={styles.infoFlex}>
              <p className={`${styles.coinSymbol} ${styles.tdP}`}>
                {coin.symbol}
              </p>
          ...
          {coin.price_change_percentage_24h >= 0 ? (
            ...
            <td>
              <div className={styles.chipFlex}>
                ...
                <div
                  className={`${styles.chipIcon} ${styles.tdChipIcon} ${styles.red}`}
                >
        ...
        <Tooltip title="Coin Price In USD" placement="bottom-end">
          {coin.price_change_percentage_24h >= 0 ? (
            ...
            <td
              className={`${styles.currentPriceRed} ${styles.tdCurrentPrice}`}
            >
              ${coin.current_price.toLocaleString()}
            </td>
          )}
        ...
        <Tooltip title="Coin Total Volume" placement="bottom-end">
          <td className={`${styles.coinName} ${styles.tdTotalVolume}`}>
        ...
        <td className={`${styles.coinName} ${styles.mobile}`}>
          ${convertNumber(coin.market_cap)}
        </td>
        <td
          className={`${styles.watchlistIcon} ${coin.price_change_percentage_24h < 0 && styles.watchlistIconRed}`}
          onClick={(e) => {
            if (isCoinAdded) {
